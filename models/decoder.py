...
class Decoder(nn.Module):
    def __init__(self, attribute_num, max_len, d_model, ffn_hidden, n_head, n_layers, drop_prob, device):
        super().__init__()
        decoders=[]
        self.device=device
        ...
        for i in range(attribute_num):
            if i == 0:
                decoders.append(Decoder_single_act(max_len, d_model, ffn_hidden, n_head, n_layers, drop_prob, device))
            else:
                decoders.append(
                    Decoder_single_attr(max_len, d_model, ffn_hidden, n_head, n_layers, drop_prob, device))
    ...
    def forward(self, Xs, enc_output,mask):
        ...
        for i in range(self.attribute_num):
            X = Xs[i]
            src_mask = mask.unsqueeze(1).unsqueeze(1).repeat((1, 1, 1, self.attribute_num))
