...
class Encoder(nn.Module):
    def __init__(self, attribute_num, max_len, d_model, ffn_hidden, n_head, n_layers,n_layers_agg, drop_prob, device):
        super().__init__()
        first_encoders=[]
        self.attribute_num=attribute_num
        for i in range(attribute_num):
            first_encoders.append(Encoder_single(max_len, d_model, ffn_hidden, n_head, n_layers, drop_prob, device))
        aggregate_encoders=[]
    ...
    def forward(self, Xs, src_mask):
        ...
        agg_mask = src_mask.repeat((1,1, 1, self.attribute_num))
        for i in range(self.attribute_num):
